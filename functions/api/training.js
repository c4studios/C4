 ...
 * POST /api/training
 *
 * Handles C4Sight workshop enquiries (the training enquiry form).
 * Validates input, checks honeypot + Turnstile, rate-limits, then sends a
 * formatted email via Resend. Mirrors functions/api/inquiries.js.
 *
 ...
 *   TURNSTILE_SECRET_KEY – Cloudflare Turnstile secret (skipped in local dev)
 */
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // --- Honeypot check ---
    if (body._gotcha) {
      return json({ success: true }); // silently accept bots
    }
    ...
    // --- Rate limit ---
    const allowed = await checkRateLimit(request, 'training');
    if (!allowed) {
    ...
    if (!message || typeof message !== 'string' || message.trim().length < 5) {
      errors.push('A short message is required (min 5 characters).');
    }
    if (errors.length > 0) {
    ...
    const clean = {
      ...
      organisation: sanitise(body.organisation || '', 200),
      sector: sanitise(body.sector || '', 80),
      format: sanitise(body.format || '', 80),
      group_size: sanitise(body.group_size || '', 80),
      message: sanitise(message),
    };

    // --- Build email ---
    const subject = clean.organisation
      ? `New C4Sight enquiry from ${clean.name} (${clean.organisation})`
      : `New C4Sight enquiry from ${clean.name}`;

    const html = buildEnquiryEmail(clean);
    ...
    console.error('Training enquiry handler error:', err);
...
function buildEnquiryEmail(data) {
  ...
  const rows = [
    ...
    data.format && ['Format', e(data.format)],
    data.group_size && ['Group size', e(data.group_size)],
  ].filter(Boolean);

  return `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:560px;margin:0 auto;padding:24px;">
      <h2 style="font-size:18px;font-weight:600;margin:0 0 20px;color:#111;">New C4Sight Workshop Enquiry</h2>
      ...
      <p style="margin-top:24px;font-size:11px;color:#aaa;">Sent from the C4Sight enquiry form on c4studios.com.au</p>
    </div>
  `;
}
